...
export default function Dashboard() {
  ...
  useEffect(() => {
    // If no store is connected, redirect to the login/connect page
    if (shopDomain === null) {
      // Don't run immediately on mount to prevent hydration mismatch, but soon enough
      const timer = setTimeout(() => router.push('/'), 100);
      return () => clearTimeout(timer);
    }
    
    if (shopDomain) {
      // Fetch system data on mount using the connected domain
      fetch('/api/system', {
        method: 'POST',
